...
pub const ASPECT_RATIO: f32 = (160 as f32) / (144 as f32);

fn generate_pixel_vertices() -> Vec<[f32; 3]> {
    let mut buf: Vec<[f32; 3]> = vec![];
    for y in 0..HEIGHT {
        for x in 0..WIDTH {
            // let pixel = self.pixels[self.index_for_coord(x, y, self.width) as usize];
            // First triangle
            buf.push([x as f32, y as f32, 0.0]);
            buf.push([(x + 1) as f32, y as f32, 0.0]);
            buf.push([x as f32, (y + 1) as f32, 0.0]);

            buf.push([x as f32, (y + 1) as f32, 0.0]);
            buf.push([(x + 1) as f32, y as f32, 0.0]);
            buf.push([(x + 1) as f32, (y + 1) as f32, 0.0]);
        }
    }
    // assert!(buf.len() == 23_345);
    return buf;
}

pub fn screen(mut commands: Commands, mut meshes: ResMut<Assets<Mesh>>) {
    let mut grid = Mesh::new(PrimitiveTopology::TriangleList);
    let vertices = generate_pixel_vertices();
    grid.insert_attribute(Mesh::ATTRIBUTE_POSITION, vertices);

    // And a RGB color attribute as well
    let mut rng = rand::thread_rng();

    let mut v_color: Vec<u32> = vec![];
    for _ in 0..(WIDTH * HEIGHT * 6) {
        let color: u32 = Color::rgb(
    ...
    for i in 0..(WIDTH * HEIGHT) {
        let scaled = i * 6;
        indices.push(scaled as u32);
        indices.push((scaled + 1) as u32);
        indices.push((scaled + 2) as u32);
        indices.push((scaled + 3) as u32);
        indices.push((scaled + 4) as u32);
        indices.push((scaled + 5) as u32);
    }
